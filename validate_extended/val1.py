...
def mllp_send(host: str, port: int, payload: bytes, timeout: float = 10.0,
    ...
    if tls:
        ...
        try:
            ...
            logger.error(f"TLS wrap failed: {e}")
            raw.close()
            return None
    try:
        ...
        while True:
            try:
                ...
                return None
            if not chunk:
                logger.error("Connection closed without response")
                return None
            buf.extend(chunk)
...
def simple_edifact_from_order(order: Dict) -> str:
    ...
    msg_ref = order.get("message_ref", f"MSG{uuid.uuid4().hex[:8].upper()}")
    lines = []
    lines.append("UNA:+.? '")
    ...
    for p in parties:
        ...
        if name:
            # Adjusted CTA for buyer/supplier contact
            lines.append(f"CTA+BY+{name}'")
    ...
    try:
        ...
        logger.warning(f"Total calculation failed: {e}; using 0.00")
        total = 0.0
...
def save_generated_message(text: str, filename: Optional[str] = None) -> str:
    if filename is None:
        filename = f"edi_{datetime.utcnow().strftime('%Y%m%d%H%M%S')}_{uuid.uuid4().hex[:4]}.edi"
    path = os.path.join(GEN_DIR, filename)
    with open(path, "wb") as f:
        f.write(text.encode("utf-8"))
    logger.info(f"Generated EDIFACT file: {path}")
    return path
...
def basic_validate_edifact_bytes(data: bytes) -> Tuple[bool, str]:
    ...
    try:
        ...
        try:
            ...
            if reported_count != seg_count:
                return False, f"segment_count_mismatch_reported={reported_count}_actual={seg_count}"
        except Exception:
            return False, "invalid_UNT_format"
        # Check UNZ: find UNZ after UNT, parse count
        unz_idx = next((i for i, ln in enumerate(parts[unt_idx:], start=unt_idx) if ln.startswith("UNZ+")), None)
        if unz_idx is None:
            return False, "missing_UNZ_after_UNT"
        unz_line = parts[unz_idx]
        try:
            unz_body = unz_line.split("+")
            reported_msg_count = int(unz_body[1])
            if reported_msg_count != 1:
                return False, f"unz_message_count_mismatch_reported={reported_msg_count}_expected=1"
        except Exception:
            return False, "invalid_UNZ_format"
    except StopIteration:
        return False, "structure_missing_segments"
    return True, "ok"
...
def move_to_ready(path: str, force: bool = False) -> Tuple[str, str]:
    ...
    logger.info(f"Moved to ready: {dest}")
...
def send_one_file(filepath: str, host: str, port: int, timeout: float = 15.0,
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            ...
            logger.error(f"Read error on attempt {attempt}: {e}")
            return False, meta
        ...
        if resp is None:
            ...
            logger.warning(f"Send failed on attempt {attempt}: no ACK or timeout")
            time.sleep(1)
            continue
        try:
            ...
            ack_text = "<binary_ack>"
        meta["ack"] = ack_text
        logger.info(f"Send successful on attempt {attempt}: ACK received")
        return True, meta
...
def process_ready_queue(host: str, port: int, timeout: float = 15.0, tls: bool = False,
    ...
    files = sorted([f for f in os.listdir(READY_DIR) if f.lower().endswith(".edi")])
    if not files:
        logger.info("No files in ready queue")
        return
    for fname in files:
        ...
        if not ok:
            ...
            try:
                ...
                logger.error(f"Meta move failed: {e}")
            logger.warning(f"Validation failed for {fname}: {prev}")
            continue
        ...
        if success:
            ...
            try:
                ...
                logger.error(f"Meta move failed: {e}")
            logger.info(f"Sent successfully: {fname}")
            if archive_on_sent:
                try:
            ...
            try:
                ...
                logger.error(f"Meta move failed: {e}")
            logger.error(f"Send failed for {fname}: {meta.get('error', 'unknown')}")

def list_dir_with_meta(directory: str) -> List[Tuple[str, Optional[Dict]]]:
    out = []
    for name in sorted(os.listdir(directory)):
        if not (name.lower().endswith(".edi") or name.lower().endswith(".meta.json")):
            continue
        ...
        if name.lower().endswith(".meta.json"):
            try:
                ...
                meta = {"_meta_error": f"failed_to_load: {e}"}
        else:
            # For .edi, try to load associated meta
...
def cli_generate(args):
    ...
    if args.input:
        ...
        logger.info("No input JSON provided; creating a minimal demo message.")
        data = {
            "message_ref": f"MSG{uuid.uuid4().hex[:8].upper()}",
...
def cli_validate(args):
    ...
    if os.path.isdir(target):
        files = [os.path.join(target, f) for f in os.listdir(target) if f.lower().endswith(".edi")]
    else:
        files = [target]
    for f in files:
